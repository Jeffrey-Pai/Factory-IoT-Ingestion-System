...
> 📌 `telemetry_worker_buffer_depth` 取代了早期的 `telemetry_channel_depth` —— 兩者量的是同一件事，
> 統一成前者是因為 `grafana/` 底下的儀表板與告警規則讀的是這個名字。
...
- `rate(telemetry_consumed_total)` ≈ `rate(telemetry_written_total)` ≈ 發布速率，且 `telemetry_failed_total` 保持為 0
- `rabbitmq_queue_messages_ready` 貼著 0，`rabbitmq_queue_consumers` ≥ 1
- `telemetry_worker_buffer_depth` 貼近 0 —— 持續偏高代表資料庫寫入跟不上
